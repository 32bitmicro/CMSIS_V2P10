 ...
 * globals.h
 * - Global header file.
 * - Contains all system and application settings for the RPM sample application
 * ----------------------------------------------------------------------------
 * $Revision: 1.7 $
...
/* Oscillator control configuration */
...
/* Configure the ADC decimation filter */
...
#define AFE_RTC_CTRL_SET_CFG    (RTC_ENABLE   | \
                                 ALARM_ENABLE | \
                                 RTC_SET_MODE)
...
/* RTC alarm timeout value - 10 seconds (RTC CLK is 16Hz) */
#define AFE_RTC_ALARM_CFG       ((16 * 10) << AFE_RTC_ALARM_RTC_ALARM_Pos | \
                                 ALARM_RELATIVE_MODE)
...
/* Set SYSTICK timer counter for 250ms, asssuming a system clock rate of
 * 3.0 MHz */
#define SYSTICK_RELOAD_COUNT        (750000)

/* ----------------------------------------------------------------------------
 * General Purpose Timer Configuration
 * ------------------------------------------------------------------------- */

#define TIMER0_CTRL_CFG_NO_TIMEOUT  (TIMER0_COUNT_1         | \
                                     TIMER0_FREE_RUN        | \
                                     TIMER0_PRESCALE_128)

/* TIMER0 config - configure for free-run mode and prescaler of 128 */
#define TIMER0_CTRL_CFG     (TIMER0_CTRL_CFG_NO_TIMEOUT     | \
...
/* TIMERS_STATUS config - disable all timers */
 ...
 * approximately 5 ms assuming a system clock of 3.0 MHz and a timer clock
 ...
 * */
#define TIMER0_TIMEOUT_VALUE        (293)

/* The number of timeouts that should occur (to total 20 ms) */
#define TIMER0_NUM_TIMEOUTS          4
...
#define GPIO_IF5_FUNCTION_CFG       (GPIO_IF5_PIN3_PULLDOWN_ENABLE | \
...
/* GPIO Input control configuration */
...
#define GPIO_INT_CTRL_GP1_CFG       (GPIO_GP1_INT_NEG_EDGE_TRIGGER_SHORT   | \
                                     GPIO_IF5_PIN0)
...
/* The default system clock speed is defined by SYS_FREQ above */
#define UART_SYSCLOCK_SPEED_DEFAULT SYS_FREQ

/* ----------------------------------------------------------------------------
 * USB Controller Configuration
 * ------------------------------------------------------------------------- */

#define USB_CFG                     (USB_RESET_ENABLE | USB_ENABLE)
...
/* Callback indicating that the ADC capture is complete. */
...
/* Application information returned by the version info command. */
#define APP_VENDOR_ID           0
#define APP_PRODUCT_ID          1

/* ----------------------------------------------------------------------------
 * Global variables
 * ------------------------------------------------------------------------- */
extern unsigned int g_SW1_Pressed;
extern unsigned int g_Num_RTC_Ticks;
