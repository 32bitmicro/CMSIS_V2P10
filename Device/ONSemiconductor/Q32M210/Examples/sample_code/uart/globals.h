...
/* TIMER0 timeout value -  indicates how frequently data is written to DAC1.
 * Configure the timeout for 1 ms.  This means that the 16 point sine wave
 * will have a period of approximately 1 ms * 16 = 16 ms. */

/* Calculate the timeout value according to:
 *
 *      count = T * (sys_clk/prescaler)
 *
 * where T is the desired timeout in seconds.
 *
 * The calculated count value would be 28 if T = 0.001 s.
 * Note that the timeout value is only 12-bits. So the maximum it can represent
 * is:
 *      2^12 - 1 = 4095
 * */

#define TIMER0_TIMEOUT_VALUE        (28)
...
#define GPIO_IF23_FUNCTION_CFG      (GPIO_IF3_PIN1_PULLUP_DISABLE   | \
                                     ...
                                     GPIO_IF3_SELECT_UART1          | \
                                     GPIO_IF2_SELECT_UART0)
...
/* Both UART 0 and UART 1 are configured to 9600 baud.  This means that
 * 960 bytes are received per second.  So, we should expect a UART interrupt
 * approximately every 1/960 s = 1 ms. */

/* UART0 configuration - no DMA, enable UART0, and disable the prescaler */
#define UART0_CTRL_CFG              (UART_CONTROLLER_CM3       | \
                                     UART_ENABLE               | \
                                     UART_PRESCALE_DISABLE)

/* UART0 default speed configuration - 9600 baud */
#define UART0_SPEED_DEFAULT         (9600)

/* Set UART 0 clock divisor - divisor set to 1 */
#define UART0_CLK_DIV_CFG           UART0_CLK_DIV_1_BYTE

/* UART1 configuration - no DMA, enable UART1, and disable the prescaler */
#define UART1_CTRL_CFG              (UART_CONTROLLER_CM3       | \
                                     UART_ENABLE               | \
                                     UART_PRESCALE_DISABLE)

/* UART1 default speed configuration - 9600 baud */
#define UART1_SPEED_DEFAULT         (9600)
...
extern void UART_Read(unsigned char data);

/* ----------------------------------------------------------------------------
 * Application-specific definitions
 * ------------------------------------------------------------------------- */

/* UART Read/Write buffer sizes are 256 */
#define BUFFER_SIZE (256)

/* Number of elements in the sine wave */
#define SINE_SIZE   (17)
...
extern int write_buffer_start;
extern int write_buffer_end;
extern int read_buffer_start;
extern int read_buffer_end;
extern int output_stored_content_flag_UART0;

#if (BUILD_TYPE==UART_LOOPBACK)
    extern char sine[SINE_SIZE];
    extern int sine_amplitude;
    extern int idx;
#endif
