...
/* Supply power to the RC oscillator and the ADCs/DACs */
...
/* Configure PGA inputs by selecting VSS for channel B (differential input)
...
/* DMA Channel 0 configuration - Transfer from ADC0 to DMA */
...
/* TIMER0 config - configure for free-run mode and prescaler of 128 */
#define TIMER0_CTRL_CFG     (TIMER0_COUNT_1         | \
                             TIMER0_FIXED_COUNT_RUN | \
                             TIMER0_PRESCALE_128    | \
                             TIMER0_TIMEOUT_VALUE)
...
/* TIMERS_STATUS config - all timers stopped */
#define TIMERS_STATUS_CFG   (TIMER0_STOP  | \
                             TIMER1_STOP  | \
                             TIMER2_STOP  | \
                             TIMER3_STOP)

/* TIMER0 timeout value - This timeout value should correspond to a period of
 * approximately 142 ms assuming a system clock of 3.0 MHz and a timer clock
 * prescaler of 128. Calculated as:
 *
 *      count = T * (sys_clk/prescaler)
 *
 * where T is the desired timeout in seconds.
 *
 * The calculated count value would be 3328. */
#define TIMER0_TIMEOUT_VALUE        (3328)

/* The number of timeouts that should occur */
#define TIMER0_NUM_TIMEOUTS          9
...
/* Output enable config – enable all GPIOs as outputs except IF4.13 */
#define GPIO_IF4_OUTPUT_ENABLE_CFG  ~(1 << GPIO_IF4_PIN13)

/* Pull-down configuration - enable pulldown for IF4.13 (SW4) */
#define GPIO_IF4_PULLDOWN_CFG       (1 << (GPIO_IF4_PIN13 << \
                                           GPIO_IF4_LCD_PULLDOWN_ENABLE_Pos))
...
/* GPIO Input control configuration - enable inputs on IF4 */
...
/* Interrupt control configuration - enable GPIO GP1 interrupt to trigger on the
 * positive edge (using SW4) */
#define GPIO_INT_CTRL_GP0_CFG       (GPIO_GP0_INT_DISABLE_SHORT)
#define GPIO_INT_CTRL_GP1_CFG       (GPIO_GP1_INT_POS_EDGE_TRIGGER_SHORT   | \
                                     GPIO_IF4_PIN13)
...
/* UART0 default speed configuration - 0 baud */
...
/* UART1 default speed configuration - 0 baud */
...
/* The default system clock speed is 3000000 hertz or 3 MHz */
#define SYSTEM_CLK_FREQ             (3000000)

/* The bit-fields used by the EEPROM library are identical for both SPI
 * interfaces. General SPI prescalar defined using SPI interface 0. */
#define SPI_PRESCALER               SPI_PRESCALE_32

/* The index of the SPI interface the EEPROM is connected to (i.e. 0 or 1) */
#define SPI_INTERFACE               (0)

/* Initial configuration for EEPROM */
#define EEPROM_CONFIG               (EEPROM_WRITEPROTECT_DISABLE | \
                                     EEPROM_BLOCKPROTECT_NONE    | \
 ...
 * Global Variables
 * ------------------------------------------------------------------------- */
extern unsigned short EEPROM_read_addr;
extern unsigned short EEPROM_write_addr;
extern unsigned short max_EEPROM_addr;
extern unsigned int DMA_Ch0_Dest_Buffer[DMA_CH0_TRANSFER_LENGTH];
extern unsigned int Timer0_Running;
extern unsigned int Timer0_Timeouts;
